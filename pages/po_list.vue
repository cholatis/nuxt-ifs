    ...
    <div>
        ...
        <div class="bg-primary text-white rounded-xl p-4 flex items-center justify-between mb-5 shadow-lg">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="text-xl font-bold">Purchase Order List</h4>
        ...
        <div class="panel mb-4 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex-grow max-w-md relative">
                    <input v-model="search" type="text" class="form-input ltr:pr-10 rtl:pl-10" placeholder="PO Number / Supplier / Buyer"
        ...
        <div v-else class="panel pb-0 shadow-sm overflow-hidden">
            <!-- Empty -->
            <div v-if="poList.length === 0" class="py-16 text-center text-white-dark text-sm">
                ไม่พบรายการ PO ที่ตรงกับเงื่อนไข
            </div>
                    ...
                    <table class="table-hover table whitespace-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th @click="setSort('po_date')" class="cursor-pointer select-none">
                                    PO Date <span class="ml-1 text-xs">{{ sortIcon('po_date') }}</span>
                                </th>
                        ...
                        <tbody>
                            <tr v-for="po in sortedPoList" :key="po.id">
                                <td class="font-semibold text-primary">{{ po.po_no }}</td>
                                <td class="text-white-dark text-sm">{{ formatDate(po.po_date) }}</td>
                                <td>{{ po.supplier_name ?? '-' }}</td>
                                <td>{{ po.buyer_name ?? '-' }}</td>
                                <td class="text-right font-bold text-primary">{{ formatCurrency(po.amount_incl_vat) }}</td>
                                <td class="text-center">{{ po.po_payment_term ?? '-' }} days</td>
                                <td class="text-center">
                                    <span :class="['badge capitalize', getStatusColor(po.status)]">{{ po.status }}</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
<script setup lang="ts">
    ...
    const fetchPOs = async () => {
        ...
        try {
            ...
            const res  = await fetch(`${LISTALLPO_URL}?${params}`, {
                headers: { Authorization: `Bearer ${jwt}` },
            });
            const json = await res.json();
            if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);

            poList.value   = json.data  ?? [];
            totalRows.value = json.total ?? 0;
        } catch (err: any) {
            fetchError.value = err.message;
            console.error('[listallpo] error:', err.message);
        } finally {
    ...
    watch([currentPage, pageSize], fetchPOs);
    onMounted(fetchPOs);
    ...
    const getStatusColor = (status: string) => {
        ...
        if (s === 'invoiced') return 'badge-outline-success';
    ...
    const exportExcel = async () => {
        try {
            ...
            // Build CSV
            const headers = ['PO Number', 'PO Date', 'Supplier Code', 'Supplier Name', 'Buyer Code', 'Buyer Name', 'Amount (excl. VAT)', 'VAT', 'Amount (incl. VAT)', 'Term (Days)', 'Status'];
            ...
            a.href     = url;
            a.download = `po_list_${new Date().toISOString().slice(0, 10)}.csv`;
            a.click();
