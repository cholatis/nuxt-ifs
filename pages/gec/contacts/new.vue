<template>
    <div>
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-dark dark:text-white">New Contact</h2>
                <p class="mt-1 text-sm text-white-dark">Add a new CRM contact</p>
            </div>
        ...
        <div class="panel">
            ...
            <form @submit.prevent="onSubmit" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold">Owner</label>
                    <div class="flex h-[42px] items-center rounded-md border border-[#e0e6ed] bg-[#fafafa] px-3 text-sm dark:border-[#1b2e4b] dark:bg-[#1b2e4b]">
                        <span class="font-semibold">{{ authStore.displayName }}</span>
                        <span class="ml-2 text-white-dark">(set automatically)</span>
                    </div>
                </div>
...
const onSubmit = async () => {
    ...
    try {
        ...
        errorMessage.value = err?.message || 'Failed to create contact';
    } finally {
